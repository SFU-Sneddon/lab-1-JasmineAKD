...
# the file appears to be the same after modifying the code replacing the single `>` with `>>`. It only appears the same in the Rmd window. Opening the txt file revealed that the file code with `>` was 25 lines whereas the file coded with `>>` was 50 lines, however if the code line with `>>` is re-run a second time then the file was 75 lines. This indicates that the file duplicates extending the length depending on the amount of times that it is re-run.

* What happens if you change the output file name (after `>>`) to `~/MBB243/out/genotype_head.txt`? Why do you think that is?

# with the file name change there was a message stating that there is "no such file or directory" this is possibly because adding the `/out/`to the file name changes the directory route which wasn't recognized
...
# the file is first cut to only containing the 4th column, then the pipe is used to sort the column, then another pipe is used to find the unique genotypes of the sorted column
...
# the EMPTY.txt file prints "Hello terminal!" each time that it is run as well as the date and time that the code was run.
...
###```{bash}
source ~/.bashrc #this command obtained desired result in terminal
...
#this code block has an error due to "non-numeric arguement to binary operator", this block also would not allow for knitting the file if eval=TRUE
...
CHROME_14=$(samtools faidx /local-scratch/course_files/MBB243/hg38.fa "chr14:65102304-65102339" | grep -v \>)
echo $CHROME_14
